  ...
  chrome.storage.sync.get(['links', 'tags'], function(result) {
    ...
    if (!result.links) {
      chrome.storage.sync.set({ links: [] }, function() {
        console.log('Storage initialized for links');
      });
    }
...
function saveLink(url, tab) {
  ...
  chrome.storage.sync.get(['links'], function(result) {
    ...
    const newLink = {
      ...
      name: domainName,
      url: url,
      tagId: 'general', // Default to General tag
...
console.log('Link Saver Extension created by MehrCodeLand - https://github.com/MehrCodeLand');